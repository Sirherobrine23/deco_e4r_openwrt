...
enable_qcawifi() {
	...
	config_get blockdfslist "$device" blockdfslist
    ...
    config_get sta_kickout "$device" sta_kickout
    [ -n "$sta_kickout" ] && iwpriv "$phy" sta_kickout "$sta_kickout"       
	...
	if [ "$country" = "DE" -a "$device" = "wifi1" ]; then
		iwpriv "$phy" set_bcnburst 0
	else
	...
	config_get TxBFCTL "$device" TxBFCTL
	[ -n "$TxBFCTL" ] && iwpriv "$phy" TxBFCTL "$TxBFCTL"
	...
	for vif in $vifs; do
		...
		[ "$vaptype" = "default" -o "$vaptype" = "lan" ] && iwconfig "$ifname" channel "$channel" >/dev/null 2>/dev/null
...
#		[ -n "$rts" ] && iwconfig "$ifname" rts "${rts%%.*}"
		...
		if [ "$country" != "ID" -a "$device" = "wifi1" -a "$mode" = "ap" ]; then
		...
		[ "$vaptype" = "default" ] && iwpriv "$ifname" splitmac 0
