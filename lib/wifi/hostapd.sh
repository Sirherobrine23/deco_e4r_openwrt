...
hostapd_set_bss_options() {
	...
	[ -z "$vif" ] && hostapd_get_vif_name && config_load wireless
	...
	[ -f /var/run/hostapd-$device/$ifname ] && rm /var/run/hostapd-$device/$ifname
	...
	[ -n "$wps_possible" -a -n "$config_methods" ] && {
		local model="$(uci_get system @system[0] hostname M4R)"
		config_get device_type "$vif" wps_device_type "6-0050F204-1"
		config_get device_name "$vif" wps_device_name "Whole-home Wi-Fi"
		config_get manufacturer "$vif" wps_manufacturer "TP-Link"
		config_get model_name "$vif" model_name "Deco $model"
		config_get model_number "$vif" model_number "Deco $model"
		config_get serial_number "$vif" serial_number "0"
#		config_get wps_pin "$vif" wps_pin "12345670"
		config_get wps_state "$vif" wps_state "2"
		...
		local prefix=$(uci_get wps wps uuid_prefix "284e8955-3a26-4fc8-ac2d")	
		local macaddr=$(cat /sys/class/net/${bridge}/address)
		uuid=$(echo "$macaddr" | sed 's/://g')
		[ -n "$uuid" ] && {
			append "$var" "uuid=$prefix-$uuid" "$N"
		}
	...
	then
		...
		config_get ft_resource_req_supp "$vif" ft_resource_req_supp
		[ -n "$ft_resource_req_supp" ] && append "$var" "ft_resource_req_supp=$ft_resource_req_supp" "$N"
		config_get ft_psk_generate_local "$vif" ft_psk_generate_local
		[ -n "$ft_psk_generate_local" ] && append "$var" "ft_psk_generate_local=$ft_psk_generate_local" "$N"
	fi
...
EOF
	...
	if [ $wps_vap_tie_dbdc -ne 0 ]; then
		...
		config_get_bool wps_hotplug "$vif" wps_hotplug 0
		if [ -n "$wps_possible" -a -n "$config_methods" -a "$wps_hotplug" -gt 0 ]; then
