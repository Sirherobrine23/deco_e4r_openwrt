...
__nrd_get_wlan_ifaces() {
    ...
    for iface in $vaps; do
        ...
        if [ "$radio" = "wifi0" ]; then
            if [ "$vap_type" = "lan" -a "$ap_2g" = "1" ] || [ "$vap_type" = "guest" -a "$guest_2g" = "1" ]; then
                nrd_echo "nrd 2g enable $iface" 
                WLAN_DEVICES="${WLAN_DEVICES}${WLAN_DEVICES:+","}${radio}:${iface}"
            fi
        elif [ "$radio" = "wifi1"  ]; then
            if [ "$vap_type" = "lan" -a "$ap_5g" = "1" ] || [ "$vap_type" = "guest" -a "$guest_5g" = "1" ]; then
...
__nrd_get_wlan_interface() {
    ...
    config_load nrd
    config_foreach __nrd_get_wlan_ifaces wlanif "$ap_2g" "$ap_5g" "$gst_2g" "$gst_5g" $2
